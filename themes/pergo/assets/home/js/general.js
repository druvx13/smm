...
function General(){
    ...
    this.add_funds = function(){
        ...
        $(document).on("submit", ".actionAddFundsForm", function(){
            ...
            _action       = PATH + 'add_funds/' + _payment_type +'/create_payment';
    ...
    this.generalOption = function(){
        ...
        $(document).on("submit", ".actionFormWithoutToast", function(){
            ...
            $.post(_action, _data, function(_result){
                if (is_json(_result)) {
                    _result = JSON.parse(_result);
                    setTimeout(function(){
                        alertMessage.show(_result.message, _result.status, '#alert-message-subscribe');
                    }, 1500)
                }else{
                    setTimeout(function(){
        ...
        // actionFormWithoutToast
        $(document).on("submit", ".actionContactForm", function(){
            $(".actionContactForm .btn-submit").addClass('btn-loading');
            alertMessage.hide();
            event.preventDefault();
            var _that       = $(this),
                _action     = PATH + "contact/send_message",
                _data       = _that.serialize();
                _data       = _data + '&' + $.param({token:token});
            ...
            $.post(_action, _data, function(_result){
                if (is_json(_result)) {
                    ...
                    setTimeout(function(){
                        $(".actionContactForm #resultActionForm").html(_result);
                    }, 1500)
                }
                setTimeout(function(){
                    $(".actionContactForm .btn-submit").removeClass('btn-loading');
                }, 1500)
            })
            return false;
        })
