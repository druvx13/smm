  ...
  var alertMessage = alertMessage || (function ($) {
    var $html = $('<div class="alert alert-icon content d-none" role="alert">' +
                ...
                '</div>');
    return {
      show: function (_message, _type, _dom = "") {
        if (_dom == "") {
          _dom = "#alert-message";
        }
        switch(_type) {
        ...
        $(_dom).html($html);
        $(_dom + ' .content').addClass(_type);
        $(_dom + ' .icon-symbol').addClass(_icon);
        $(_dom + ' .content').removeClass('d-none');
        $(_dom + ' .content .message').html(_message);
      },
